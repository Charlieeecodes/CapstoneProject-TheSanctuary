...
exports.addInquiry = async (req, res) => {
  ...
  const sql =
    "INSERT INTO inquiries (name, email, subject, message, status) VALUES (?, ?, ?, ?, 'Pending')";
  ...
  try {
    ...
    return res.status(500).json({ success: false, message: "Error adding inquiry" });
  }
};
...
exports.getInquiryById = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ success: false, message: "Error fetching inquiry" });
  }
};
...
// ✅ Send message (real email via SMTP)
